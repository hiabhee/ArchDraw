  ...
  return (
    <div className="rounded-xl border border-[#18191a] bg-[#0f1011] p-4">
      <div className="flex items-center gap-2 mb-2">
        ...
        <span className="text-xs text-[#8a8f98] uppercase tracking-wider">{label}</span>
      </div>
...
function BarChart({ data, labelKey, valueKey, maxBars = 10 }: { data: Record<string, string | number>[]; labelKey: string; valueKey: string; maxBars?: number }) {
  ...
  return (
    <div className="space-y-1.5">
      {items.length === 0 && <div className="text-xs text-[#62666d] py-4 text-center">No data yet</div>}
      {items.map((item, i) => (
        <div key={i} className="flex items-center gap-2 text-xs">
          <div className="w-[140px] truncate text-[#d0d6e0] shrink-0" title={String(item[labelKey])}>
            {String(item[labelKey])}
          </div>
          <div className="flex-1 h-4 bg-[#141516] rounded overflow-hidden">
            <div
              className="h-full bg-[#1E90FF] rounded transition-all duration-500"
              style={{ width: `${(Number(item[valueKey]) / maxVal) * 100}%` }}
            />
          </div>
          <div className="w-10 text-right text-[#8a8f98] tabular-nums">{Number(item[valueKey])}</div>
        </div>
      ))}
    </div>
  );
...
function MiniLineChart({ data }: { data: DailyRow[] }) {
  if (data.length === 0) return <div className="text-xs text-[#62666d] py-8 text-center">No data yet</div>;
  ...
  return (
    <div>
      <svg viewBox={`0 0 ${width} ${height}`} className="w-full h-24" preserveAspectRatio="none">
        <path d={pathD} fill="none" stroke="#1E90FF" strokeWidth="1.5" vectorEffect="non-scaling-stroke" />
        {points.map((p, i) => (
          <circle key={i} cx={p.x} cy={p.y} r="1" fill="#1E90FF" />
        ))}
      </svg>
      <div className="flex justify-between text-[10px] text-[#62666d] mt-1 px-1">
  ...
  return (
    <div className="space-y-6 max-w-6xl">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <select
            ...
            className="text-xs bg-[#0f1011] border border-[#18191a] rounded-lg px-2 py-1.5 text-[#d0d6e0] cursor-pointer"
          >
      ...
      <div className="grid md:grid-cols-2 gap-4">
        {/* Daily visitors */}
        <div className="rounded-xl border border-[#18191a] bg-[#0f1011] p-4">
          <h3 className="text-xs text-[#8a8f98] uppercase tracking-wider mb-3">Daily Visitors</h3>
          <MiniLineChart data={data?.daily || []} />
        </div>

        {/* Funnel */}
        <div className="rounded-xl border border-[#18191a] bg-[#0f1011] p-4">
          <h3 className="text-xs text-[#8a8f98] uppercase tracking-wider mb-3">
            Funnel <span className="text-[#1E90FF]">({funnelConversion}% conversion)</span>
          </h3>
          <div className="space-y-2">
            {funnel.map((row, i) => {
              ...
              return (
                <div key={i} className="flex items-center gap-2 text-xs">
                  <div className="w-[120px] text-[#d0d6e0] capitalize">{row.stage.replace(/_/g, ' ')}</div>
                  <div className="flex-1 h-4 bg-[#141516] rounded overflow-hidden">
                    <div
                  ...
                  <div className="w-10 text-right text-[#8a8f98] tabular-nums">{row.unique_visitors}</div>
                </div>
              );
            })}
            {funnel.length === 0 && <div className="text-xs text-[#62666d] py-4 text-center">No funnel data yet</div>}
          </div>
        </div>
      </div>
      ...
      <div className="grid md:grid-cols-3 gap-4">
        <div className="rounded-xl border border-[#18191a] bg-[#0f1011] p-4">
          <h3 className="text-xs text-[#8a8f98] uppercase tracking-wider mb-3">Top Pages</h3>
          <BarChart data={data?.topPages || []} labelKey="page_path" valueKey="views" />
        </div>
        <div className="rounded-xl border border-[#18191a] bg-[#0f1011] p-4">
          <h3 className="text-xs text-[#8a8f98] uppercase tracking-wider mb-3">Top Clicks</h3>
