...
import type { EndpointStats, PageMeasure, PerfSnapshot } from './measure.js';
...
const a = JSON.parse(readFileSync(resolve(aPath), 'utf8')) as PerfSnapshot;
const b = JSON.parse(readFileSync(resolve(bPath), 'utf8')) as PerfSnapshot;
...
console.log(`Bundle static gzip: ${humanBytes(a.bundle?.totalStaticGzip ?? 0)} → ${humanBytes(b.bundle?.totalStaticGzip ?? 0)} ${a.bundle && b.bundle ? delta(a.bundle.totalStaticGzip, b.bundle.totalStaticGzip) : ''}`);
console.log(`First-load gzip: ${humanBytes(a.bundle?.firstLoadApproxGzip ?? 0)} → ${humanBytes(b.bundle?.firstLoadApproxGzip ?? 0)} ${a.bundle && b.bundle ? delta(a.bundle.firstLoadApproxGzip, b.bundle.firstLoadApproxGzip) : ''}`);
console.log(`Chunks: ${a.bundle?.chunkCount ?? '—'} → ${b.bundle?.chunkCount ?? '—'}`);
console.log('');

const mapA = new Map((a.endpoints ?? []).map((e: EndpointStats) => [e.name, e]));
const mapB = new Map((b.endpoints ?? []).map((e: EndpointStats) => [e.name, e]));
...
for (const name of allNames) {
  const ea = mapA.get(name); const eb = mapB.get(name);
  if (!ea) {
    if (!eb) continue;
    console.log(`| ${name} | — | ${fmt(eb.ttfb.p95)} | new | — | ${fmt(eb.total.p95)} | new |`);
    continue;
  }
...
console.log('');
const pagesA = new Map((a.pages ?? []).map((p: PageMeasure) => [p.name, p]));
const pagesB = new Map((b.pages ?? []).map((p: PageMeasure) => [p.name, p]));
console.log('| Page | Baseline TTFB | Current TTFB | Δ |');
console.log('|---|---|---|---|');
for (const k of new Set([...pagesA.keys(), ...pagesB.keys()])) {
  const pa = pagesA.get(k); const pb = pagesB.get(k);
  if (!pa || !pb) continue;
