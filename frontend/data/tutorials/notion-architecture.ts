    ...
    level({
      ...
      steps: [
        ...
        step({
          ...
          parent: 'API Gateway',
          phases: {
            context: { heading: 'Step 3: Load Balancer', body: 'Notion routes block edits, page loads, and WebSocket sync through a load balancer that spreads traffic across stateless API pods.' },
            intro: { heading: 'About Load Balancers', body: 'Load balancers distribute requests across many server instances for availability and throughput.' },
            teaching: { heading: 'Deep dive: Load Balancer', body: 'Workspace traffic spikes when teams share a doc during meetings. The load balancer keeps p99 latency stable by shedding unhealthy pods and coalescing duplicate reads for the same page version.' },
        ...
        step({
          ...
          phases: {
            ...
            connecting: { heading: 'Connect it up', body: 'Connect Load Balancer \u2192 Block Service.' },
            celebration: { heading: 'Great job!', body: 'Block Service added.' },
          },
          hints: ['Search for "Block Service"', 'Connect Load Balancer to it'],
        }),
      ],
    }),
    level({
      ...
      steps: [
        ...
        step({
          ...
          parent: 'Block Service',
          phases: {
            context: { heading: 'Level 2: In-Memory Cache', body: 'Hot pages and block trees are cached in Redis so repeated edits and scrolls do not hammer the database.' },
            intro: { heading: 'About Caches', body: 'In-memory caches store frequently accessed block trees for sub-millisecond reads.' },
            teaching: { heading: 'Deep dive: In-Memory Cache', body: 'When ten collaborators open the same wiki page, the first read populates Redis; subsequent block fetches avoid Postgres entirely. Cache invalidation is keyed by page version so OT merges never serve stale trees.' },
            action: { heading: 'Your turn!', body: "Press \u2318K, search for 'In-Memory Cache', and add it." },
            connecting: { heading: 'Connect it up', body: 'Connect Block Service \u2192 In-Memory Cache.' },
            celebration: { heading: 'Great job!', body: 'Cache added.' },
          },
          hints: ['Search for "In-Memory Cache"', 'Connect Block Service to it'],
        }),
      ],
    }),
    level({
      ...
      steps: [
        step({
          ...
          phases: {
            ...
            celebration: { heading: 'Great job!', body: 'Real-time Service added.' },
          },
        ...
        step({
          ...
          parent: 'API Gateway',
          phases: {
            context: { heading: 'Level 3: Auth Service', body: 'Notion workspaces enforce role-based access — guests, members, and admins see different block trees.' },
            intro: { heading: 'About Auth Services', body: 'Auth services validate sessions and enforce workspace permissions on every block operation.' },
            teaching: { heading: 'Deep dive: Auth Service', body: 'Sharing a page link must not leak private databases in the same workspace. Auth checks happen at the gateway before block reads or OT broadcasts, preventing IDOR attacks on block UUIDs.' },
            ...
            celebration: { heading: 'Great job!', body: 'Auth Service added. Your Notion architecture is complete!' },
          },
