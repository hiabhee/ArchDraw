      ...
      steps: [
        step({
          ...
          phases: {
            ...
            action: { heading: 'Your turn!', body: "Press \u2318K, search for 'Mobile Client', and add the Mobile Client." },
          ...
          hints: ['Search for "Mobile Client"'],
        }),
        step({
        ...
        step({
          ...
          phases: {
            ...
            action: { heading: 'Your turn!', body: "Press \u2318K, search for 'Message Service', and add the Message Service." },
          ...
          hints: ['Search for "Message Service"', 'Connect API Gateway to it'],
        }),
        step({
          ...
          parent: 'API Gateway',
          phases: {
            context: { heading: 'Step 4: Load Balancer', body: 'WhatsApp spreads connection handlers across Erlang nodes — the load balancer routes each session to a healthy gateway pod.' },
            intro: { heading: 'About Load Balancers', body: 'Load balancers distribute persistent connections and API traffic across clusters.' },
            teaching: { heading: 'Deep dive: Load Balancer', body: 'New Year\'s Eve message spikes require horizontal scale. Sticky sessions keep each user on the same node for connection state while allowing failover when a rack goes dark.' },
        ...
        step({
          ...
          phases: {
            context: { heading: 'Step 5: Message Queue', body: 'Offline delivery and group fan-out use a durable queue so bursts do not drop encrypted payloads.' },
            intro: { heading: 'About Message Queues', body: 'Message queues buffer events between producers and consumers for reliable async delivery.' },
            teaching: { heading: 'Deep dive: Message Queue', body: 'When a user reconnects after a flight, the queue replays pending ciphertext in order. Group messages fan out to per-recipient topics without blocking the sender\'s UI.' },
            action: { heading: 'Your turn!', body: "Press \u2318K, search for 'Message Queue', and add it." },
            connecting: { heading: 'Connect it up', body: 'Connect Message Service \u2192 Message Queue.' },
            celebration: { heading: 'Great job!', body: 'Message Queue added. Messages can now be queued reliably.' },
          },
          hints: ['Search for "Message Queue"', 'Connect Message Service to it'],
        }),
      ],
    }),
    level({
      ...
      steps: [
        step({
          ...
          phases: {
            ...
            action: { heading: 'Your turn!', body: "Press \u2318K, search for 'Voice Service', and add the Voice Service." },
          ...
          hints: ['Search for "Voice Service"', 'Connect Mobile Client to it'],
        }),
      ],
    }),
  ],
