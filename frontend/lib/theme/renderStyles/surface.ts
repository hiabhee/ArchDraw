...
export function resolveRenderSurface(input: ResolveRenderSurfaceInput): RenderSurface {
  ...
  // Sketch: clean paper fill + hand-ink border; precision keeps clean white
  const fill = sketch
    ? input.isDark ? SKETCH_PAPER_DARK : SKETCH_PAPER_TINT
    : input.isDark ? styles.background : '#ffffff';
  // Sketch selection: muted penciled accent (not full saturated ring)
  const stroke = selected
    ...
    : sketch
      ...
      : input.isDark
        ...
        : 'rgba(15, 23, 42, 0.14)';
  const strokeWidth = selected ? (sketch ? 2.0 : 2) : sketch ? 1.35 : 1.25;
  // Sketch has no soft drop shadow — the wobbly ink is the character
  const boxShadow = sketch ? 'none' : selected ? styles.shadowSelected : styles.shadow;
  const dropShadow = sketch
    ? 'none'
    : input.isDark
      ? selected
