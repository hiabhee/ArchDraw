...
export function LandingNav({ outfitClassName }: { outfitClassName?: string }) {
  ...
  return (
    <motion.header
      ...
      className={`fixed top-0 left-0 right-0 z-50 h-14 transition-[background-color,border-color,backdrop-filter] duration-200 border-b ${
        scrolled
      ...
      <div className="max-w-[1280px] mx-auto px-6 h-full flex items-center justify-between">
        <div className="flex items-center gap-8">
          <Link href="/" className="flex items-center gap-2 group">
            <div className="w-7 h-7 rounded-lg bg-brand flex items-center justify-center flex-shrink-0 shadow-[0_0_15px_rgba(30,144,255,0.35)] transition-transform duration-200 group-hover:scale-105">
              <svg aria-hidden="true" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round">
        ...
        <div className="hidden md:flex items-center gap-3">
          <a
            ...
            className="text-sm text-[#575752] hover:text-[#1c1c1a] px-4 py-1.5 rounded-lg border border-[#e4e4df] bg-white hover:bg-slate-50 transition-[color,background-color,border-color] duration-150 font-medium"
          >
            Sign in
          </a>
          <a
